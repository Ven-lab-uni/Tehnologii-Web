// Database.js - Sistem Complet E-commerce cu IndexedDB
// Tehnologie: IndexedDB (baza de date client-side persistență)
// Avantaje: Fără server, date persistente între sesiuni, rapiditate

class DatabaseManager {
    constructor() {
        this.dbName = 'MyLibraryDB';           // Numele bazei de date
        this.version = 1;                      // Versiunea pentru upgrade-uri
        this.db = null;                        // Conexiunea la baza de date
        this.booksData = null;                  // Datele cărților din JSON
    }
    ...
    initDatabase() {
        return new Promise((resolve, reject) => {
            ...
            // Creare structură baze de date (doar la prima accesare)
            request.onupgradeneeded = (event) => {
                this.db = event.target.result;
                
                // Store pentru cărți (produse)
    ...
    async populateIfEmpty() {
        // Forțăm repopulare pentru a sincroniza stocurile
        const books = await this.getAllBooks();
        
        console.log(`📚 Stare actuală DB: ${books.length} cărți`);
        
        // Verificăm dacă stocurile din DB corespund cu cele din JSON
        ...
        if (books.length === 0 || needsUpdate) {
            ...
            // Ștergem toate cărțile vechi
            const clearRequest = store.clear();
            
            clearRequest.onsuccess = () => {
                console.log('🗑️ Baza de date curățată, adăugăm date noi...');
                
                // Adăugăm toate cărțile cu stocuri actualizate
    ...
    // ========================================
    // GOLIRE COȘ CU RETURNARE STOC
    // ========================================
    async clearCart() {
        // Returnăm produsele în stoc înainte de golire
        const items = await this.getCartItems();
        for (const item of items) {
            await this.updateBookStock(item.bookId, -item.quantity); // Returnăm la stoc (cantitate negativă)
        }
    ...
    createCartModal(items) {
        ...
        // Event handlers pentru închidere
    ...
    // ========================================
    // METODĂ CHECKOUT CU EMAIL CONFIRMARE
    // ========================================
    async checkout() {
        try {
            const items = await this.getCartItemsWithDetails();
            ...
            // Afișăm confirmare detaliată
            this.showOrderConfirmation(order);
            
            // Simulăm trimitere email
            this.simulateEmailConfirmation(order);
            
            // Închidem modal-ul coșului
            document.querySelector('.cart-modal')?.remove();
            this.updateCartCount();
